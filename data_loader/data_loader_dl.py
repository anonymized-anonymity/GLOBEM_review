...
class MultiSourceDataGenerator():
    ...
    def __data_generation_between(self, batch_ids_dict):
        ...
        X1, y1, dataset1 = [], [], []
        X2, y2, dataset2 = [], [], []
        length = len(batch_ids_dict[list(batch_ids_dict.keys())[0]])
...
def prep_repo_np_dict(flag_normalize = True, ds_keys_dict = None, verbose:bool = True) -> Dict[str, Dict[str, DataRepo_np]]:
    ...
    Args:
        ...
        ds_keys_dict (_type_, optional): a dictionary of ds_keys. Defaults to None.
            If none, will load global_config and use the defined list in the config
    ...
    if (ds_keys_dict is None):
        ds_keys_dict = {
            "dep_endterm": global_config["all"]["ds_keys"],
            "dep_weekly": global_config["all"]["ds_keys"],
        }
...
def data_loader_dl_placeholder(pred_targets: List[str], ds_keys_target: List[str], verbose:bool = True):
    ...
    def generate_dl_placeholder():
        ds_keys_max = [f.replace("_sample","") for f in os.listdir(path_definitions.RAWDATA_PATH)
            if not f.startswith(".") and not f.startswith("_")
                and os.path.isdir(os.path.join(path_definitions.RAWDATA_PATH, f))]
        ds_keys_max = sorted(list(set(ds_keys_max)))
        
        # TODO: could be removed if want to inlcude additional dataset
        ds_keys_max = [f for f in ds_keys_max if "INS" in f]

        dataset_dict = data_loader_ml.data_loader({pt: ds_keys_max for pt in global_config["all"]["prediction_tasks"]}, verbose=verbose)
        for pt, dsd_ds in dataset_dict.items():
            for ds, dsd in dsd_ds.items():
                dataset_dict[pt][ds].datapoints = dataset_dict[pt][ds].datapoints.iloc[:1]
...
class dl_feat_preparation():
    ...
    def __init__(self, config_name = "dl_feat_prep", verbose = 0, **kwargs):
        super().__init__()

        with open(os.path.join(path_definitions.CONFIG_PATH, f"{config_name}.yaml"), "r") as f:
            self.config = yaml.safe_load(f)

        all_feats = [f for ft in ['f_loc', 'f_screen', 'f_slp', 'f_steps'] for f in fc_repo.feature_columns_selected_epoches_types["allday"][ft]]
        all_feats += [f for ft in ['f_loc', 'f_screen', 'f_slp', 'f_steps'] for f in fc_repo.feature_columns_selected_epoches_types["morning"][ft]]
        all_feats += [f for ft in ['f_loc', 'f_screen', 'f_slp', 'f_steps'] for f in fc_repo.feature_columns_selected_epoches_types["afternoon"][ft]]
        all_feats += [f for ft in ['f_loc', 'f_screen', 'f_slp', 'f_steps'] for f in fc_repo.feature_columns_selected_epoches_types["evening"][ft]]
        all_feats += [f for ft in ['f_loc', 'f_screen', 'f_slp', 'f_steps'] for f in fc_repo.feature_columns_selected_epoches_types["night"][ft]]
    ...
    def prep_data_repo(self, dataset:DatasetDict, flag_train:bool = True) -> DataRepo:
        ...
        @globalize
        def impute(df):
            return df.fillna(df.median(axis = 0)).fillna(self.NAFILL).values
