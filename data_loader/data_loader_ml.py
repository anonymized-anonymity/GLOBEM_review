...
def data_loader_single_dataset_label_based(institution:str, phase:int,
    prediction_target:str, flag_max_feature_types:bool = False) -> pd.DataFrame:
    ...
    Args:
        ...
        flag_max_feature_types (bool, optional): whether load all sensor types. Should be False for maximum compatibility. Defaults to False.
    ...
    df_participant_file = pd.read_csv(data_factory.device_info_folder[institution][phase] + "platform.csv", low_memory=False)
    ...
    if (prediction_target == "dep_weekly"):
        ...
        raise ValueError("prediction_target not supported. Please make sure it is one of the following list: phq4, BDI_II, dep_weekly, dep_endterm")
    ...
    if not flag_max_feature_types: # maximum compatibility of multiple datasets across insitutes
...
def data_loader_single(prediction_target:str, institution:str, phase:int, flag_max_feature_types:bool = False) -> DatasetDict:
    ...
    if (os.path.exists(dataset_file_path)):
        ...
        datapoints = data_loader_single_dataset_label_based(institution, phase, prediction_target, flag_max_feature_types)
...
def data_loader(ds_keys_dict: Dict[str, List[str]], flag_max_feature_types:bool = False, verbose:bool = True) -> Dict[str, Dict[str, DatasetDict]]:
    ...
    for prediction_target, ds_keys in tqdm(ds_keys_dict.items(), position=0, desc= "loading prediction targets", disable=not verbose):
        for ds_key in tqdm(ds_keys, position=1, desc= "dataset keys", leave=False, disable=not verbose):
            ...
            phase = int(phase)
            if flag_max_feature_types: # currently only one institute can have max feature types
                assert institution in ["INS1"]
            dataset = data_loader_single(prediction_target, institution, phase, flag_max_feature_types)
