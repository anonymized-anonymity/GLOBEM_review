...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_task', default='allbutone', type=str,
        ...
        "(1) 'single' - cross-validation on a single dataset \n" + 
        "(2) 'allbutone' - leave-one-dataset-out setup \n" + 
        "(3) 'crossgroup` - cross-institute or cross-year setup \n" + 
        "(4) 'crosscovid' - pre/post COVID setup, only support certain datasets \n" + 
        "(5) 'two' - all combinations of two-dataset pairs, one as training set and the other as testing set \n" + 
        "(6) 'two_overlap' - train/test on overlapping users between two datasets \n" + 
        "(7) 'all' - do all evaluation setup from (1) to (6).\n Default 'allbutone'.")
    ...
    assert set(eval_task).issubset(set(["all", "two", "allbutone", "crossgroup", "crosscovid", "two_overlap", "single"]))
    ...
    if (config_name.startswith("dl_") or "dl_clustering" in config_name):
        ...
        dataset_dict = data_loader_ml.data_loader(
            ds_keys_dict={pt: ds_keys for pt in pred_targets}, verbose=verbose>0)
    ...
    # Only applicable for one institute INS1
    if ("crosscovid" in eval_task or "all" in eval_task and len(ds_keys) > 1):
        if (len(set(ins_list)) == 1 and ins_list[0] == "INS1"):
            if not flag_dl:
                ray.init(num_cpus=NJOB, ignore_reinit_error=True)
