...
class DepressionDetectionAlgorithm_ML_basic(DepressionDetectionAlgorithmBase):
    ...
    def __init__(self, config_dict:dict = None, config_name:str = "canzian"):
        ...
        for save_file in save_files:
            key = os.path.basename(save_file).replace(".pkl","")
            with open(save_file , "rb") as f:
                self.results_reload_dict[key] = pickle.load(f)

    def prep_X_reload(self, key: str) -> Tuple[bool, pd.DataFrame]:
        ...
        if not self.config["training_params"]["save_and_reload"]:
            return False, None
        key_list = key.split(":")
        flag_seperate_reload = set(key_list).issubset(set(self.results_reload_dict.keys()))
        if (flag_seperate_reload):
            X_tmp_list = []
            for key in key_list:
                print("reload, ", key)
                X_tmp_list.append(self.results_reload_dict[key])
            X_tmp_reload = pd.concat(X_tmp_list)
        else:
            X_tmp_reload = None
        return flag_seperate_reload, X_tmp_reload

    def prep_X(self, key:str, df_datapoints: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        self.save_file_path = os.path.join(self.results_save_folder, key.replace(":", "-") + ".pkl")
        ...
        if (self.config["training_params"]["save_and_reload"] and os.path.exists(self.save_file_path)):
            ...
            flag_seperate_reload, X_tmp_reload = self.prep_X_reload(key)
            if flag_seperate_reload:
                X_tmp = X_tmp_reload
            else:
    ...
    def prep_data_repo(self, dataset:DatasetDict, flag_train:bool = True) -> DataRepo:
        ...
        X_tmp = self.prep_X(key = dataset.key, df_datapoints=df_datapoints)
